var config = {
	...
	databaseURL: 'https://train-schedule-e5dfb.firebaseio.com/',
...
$('#click-button').on('click', function() {
	...
	// Grabs user input
	var trainNameForm = $('#trainNameForm')
		.val()
		.trim();
	var destinationForm = $('#destinationForm')
		.val()
		.trim();
	var trainTimeForm = moment(
		$('#trainTimeForm')
			.val()
			.trim(),
		'HH:mm'
	...
	var frequencyForm = $('#frequencyForm')
		.val()
		.trim();
...
database.ref().on('child_added', function(childSnapshot, prevChildKey) {
	...
	$('#trainScheduleTable > tbody').append(
		'<tr><td>' +
			trainName +
			'</td><td>' +
			trainDestination +
